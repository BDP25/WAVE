...
def get_clusters_per_date(date: str):
    ...
    try:
        ...
        cursor.execute(
            "SELECT cluster_id, wikipedia_article_names FROM cluster_based_summary WHERE date = %s",
            (date,)
        )
...
def get_article_info(article_id):
    ...
    try:
        ...
        cursor.execute(
            "SELECT * FROM artikel_based_summary WHERE article_id = %s",
            (article_id,)
        )
...
def get_min_max_date():
    ...
    try:
        ...
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute(
            "SELECT MIN(date) AS oldest_date, MAX(date) AS newest_date FROM cluster_based_summary"
        )
