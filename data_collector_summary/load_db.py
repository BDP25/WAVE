...
def create_schema(db_params=None):
    ...
    commands = [
        """
        CREATE TABLE IF NOT EXISTS Cluster_based_summary (
        ...
        """,
        """
        CREATE TABLE IF NOT EXISTS Artikel_based_summary (
            ...
            CONSTRAINT fk_cluster
                ...
                REFERENCES Cluster_based_summary(cluster_id)
                ON DELETE SET NULL
        )
        """
    ]
...
def load_data(json_input, db_params):
    ...
    try:
        ...
        for cluster in cluster_records:
            cur.execute(
                """
                INSERT INTO Cluster_based_summary (cluster_id, wikipedia_article_names, date, summary_text)
                VALUES (%s, %s, %s, %s)
        ...
        for artikel in artikel_records:
            cur.execute(
                """
                INSERT INTO Artikel_based_summary (article_id, cluster_id, pubtime, medium_name, head, content, article_link)
